...
// Покомпонентное умножение вектора на вектор
Vector operator*(Vector v1, const Vector& v2) {
    assert(v1.size() == v2.size());
...
//Евклидова разность для расстояния между точками
Real Euclidean_metric(const Vector& v1, const Vector& v2){
	assert(v1.size() == v2.size());
	double ans=0;
